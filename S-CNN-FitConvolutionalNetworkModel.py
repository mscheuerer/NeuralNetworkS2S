...
z500_fcst_1deg = z500_fcst
tcw_fcst_1deg = tcw_fcst
z500_fcst = (z500_fcst_1deg[:,:,:,:,0:ny:2,0:nx:2]+z500_fcst_1deg[:,:,:,:,1:ny:2,0:nx:2]+z500_fcst_1deg[:,:,:,:,0:ny:2,1:nx:2]+z500_fcst_1deg[:,:,:,:,1:ny:2,1:nx:2])/4.
tcw_fcst = (tcw_fcst_1deg[:,:,:,:,0:ny:2,0:nx:2]+tcw_fcst_1deg[:,:,:,:,1:ny:2,0:nx:2]+tcw_fcst_1deg[:,:,:,:,0:ny:2,1:nx:2]+tcw_fcst_1deg[:,:,:,:,1:ny:2,1:nx:2])/4.
ny = ny//2
nx = nx//2
...
f5 = np.load("/home/michael/Desktop/CalifAPCP/tuning/cnn-2deg-m"+str(imod)+"-drpt-f48.npz")
...
for iyr in range(0,20):
    ...
    ### Save out to file
    outfilename = "/home/michael/Desktop/CalifAPCP/forecasts/CNN-rv/probfcst_cnn-m"+str(imod)+"-drpt-2deg_yr"+str(iyr)
