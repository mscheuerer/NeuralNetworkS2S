...
def build_cat_model(n_features, hidden_nodes, n_bins, par_reg):
    ...
    x = Dense(hidden_nodes[0], activation='elu', kernel_regularizer=regularizers.l1(par_reg))(inp)
...
for iyr in range(20):
    ...
    model.fit(train_predictors, train_cat_targets, epochs=100, batch_size=ndts*nyrs_tr*nxy, verbose=0)
    # Calculate probability forecasts
    verif_predictors = np.concatenate((lon_verif,lat_verif,apcp_efi_verif),axis=2).reshape((-1,3))
    prob_fcst_cat = model.predict(verif_predictors).reshape((ndts,nxy,ncat))
    ### Save out to file
    outfilename = "/home/michael/Desktop/CalifAPCP/forecasts/ANN-rv/probfcst_10-l1_"+clead+"_yr"+str(iyr)
